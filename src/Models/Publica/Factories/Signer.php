...
class Signer
{
    ...
    private static function createSignature(
        ...
        $signatureNode = $dom->createElementNS($nsDSIG, self::PREFIX.'Signature');
        $root->appendChild($signatureNode);
        $signedInfoNode = $dom->createElement(self::PREFIX.'SignedInfo');
        $signatureNode->appendChild($signedInfoNode);
        $canonicalNode = $dom->createElement(self::PREFIX.'CanonicalizationMethod');
        $signedInfoNode->appendChild($canonicalNode);
        $canonicalNode->setAttribute('Algorithm', $nsCannonMethod);
        $signatureMethodNode = $dom->createElement(self::PREFIX.'SignatureMethod');
        $signedInfoNode->appendChild($signatureMethodNode);
        $signatureMethodNode->setAttribute('Algorithm', $nsSignatureMethod);
        $referenceNode = $dom->createElement(self::PREFIX.'Reference');
        $signedInfoNode->appendChild($referenceNode);
        if (!empty($idSigned)) {
            $idSigned = "#$idSigned";
        }
        $referenceNode->setAttribute('URI', $idSigned);
        $transformsNode = $dom->createElement(self::PREFIX.'Transforms');
        $referenceNode->appendChild($transformsNode);
        $transfNode1 = $dom->createElement(self::PREFIX.'Transform');
        $transformsNode->appendChild($transfNode1);
        $transfNode1->setAttribute('Algorithm', $nsTransformMethod1);
        $transfNode2 = $dom->createElement(self::PREFIX.'Transform');
        $transformsNode->appendChild($transfNode2);
        $transfNode2->setAttribute('Algorithm', $nsTransformMethod2);
        $transfNode3 = $dom->createElement(self::PREFIX.'Transform');
        $xPath = $dom->createElement(self::PREFIX.'XPath', 'not(ancestor-or-self::ds:Signature)');
        $transfNode3->appendChild($xPath);
        $transformsNode->appendChild($transfNode3);
        $transfNode3->setAttribute('Algorithm', $nsTransformMethod3);
        $digestMethodNode = $dom->createElement(self::PREFIX.'DigestMethod');
        $referenceNode->appendChild($digestMethodNode);
        $digestMethodNode->setAttribute('Algorithm', $nsDigestMethod);
        $digestValueNode = $dom->createElement(self::PREFIX.'DigestValue', $digestValue);
        $referenceNode->appendChild($digestValueNode);
        $c14n = self::canonize($signedInfoNode, $canonical);
        $signature = $certificate->sign($c14n, $algorithm);
        $signatureValue = base64_encode($signature);
        $signatureValueNode = $dom->createElement(self::PREFIX.'SignatureValue', $signatureValue);
        $signatureNode->appendChild($signatureValueNode);
        $keyInfoNode = $dom->createElement(self::PREFIX.'KeyInfo');
        $signatureNode->appendChild($keyInfoNode);
        $x509DataNode = $dom->createElement(self::PREFIX.'X509Data');
        $keyInfoNode->appendChild($x509DataNode);
        $pubKeyClean = $certificate->publicKey->unFormated();
        $x509CertificateNode = $dom->createElement(self::PREFIX.'X509Certificate', $pubKeyClean);
        $x509DataNode->appendChild($x509CertificateNode);

        $keyValue    = $dom->createElement(self::PREFIX.'KeyValue');
        $rsaKeyValue = $dom->createElement(self::PREFIX.'RSAKeyValue');
        $modulus     = $dom->createElement(self::PREFIX.'Modulus', $certificate->privateKey->modulus());
        $exponent    = $dom->createElement(self::PREFIX.'Exponent', $certificate->privateKey->expoent());
    ...
    public static function removeSignature($content)
    {
        ...
        $signature = $node->getElementsByTagName(self::PREFIX.'Signature')->item(0);
        if (!empty($signature)) {
    ...
    public static function existsSignature($content)
    {
        ...
        $dom->preserveWhiteSpace = false;
        $dom->loadXML($content);
        $signature = $dom->getElementsByTagName(self::PREFIX.'Signature')->item(0);
    ...
    public static function signatureCheck($xml, $canonical = self::CANONICAL)
    {
        ...
        $signature = $dom->getElementsByTagName(self::PREFIX.'Signature')->item(0);
        $sigMethAlgo = $signature->getElementsByTagName(self::PREFIX.'SignatureMethod')->item(0)->getAttribute('Algorithm');
        $algorithm = OPENSSL_ALGO_SHA256;
        if ($sigMethAlgo == 'http://www.w3.org/2000/09/xmldsig#rsa-sha1') {
            $algorithm = OPENSSL_ALGO_SHA1;
        }
        $certificateContent = $signature->getElementsByTagName(self::PREFIX.'X509Certificate')->item(0)->nodeValue;
        $publicKey = PublicKey::createFromContent($certificateContent);
        $signInfoNode = self::canonize($signature->getElementsByTagName(self::PREFIX.'SignedInfo')->item(0), $canonical);
        $signatureValue = $signature->getElementsByTagName(self::PREFIX.'SignatureValue')->item(0)->nodeValue;
    ...
    public static function digestCheck($xml, $tagname = '', $canonical = self::CANONICAL)
    {
        ...
        $root = $dom->documentElement;
        $signature = $dom->getElementsByTagName(self::PREFIX.'Signature')->item(0);
        $sigURI = $signature->getElementsByTagName(self::PREFIX.'Reference')->item(0)->getAttribute('URI');
        if (empty($tagname)) {
            if (empty($sigURI)) {
        ...
        $informedDigest = $signature->getElementsByTagName(self::PREFIX.'DigestValue')->item(0)->nodeValue;
